...
void RARunnable::run()
{
    ...
    {
        sLog->outError("WCloud RA can not bind to port %d on %s", raport, stringip.c_str());
        return;
    }

    sLog->outString("Starting WCloud RA on port %d on %s", raport, stringip.c_str());
    ...
    sLog->outStaticDebug("WCloud RA thread exiting");
}
