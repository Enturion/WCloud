...
{
    CellCoord pair(WCloud::ComputeCellCoord(x, y));
    Cell cell(pair);
    cell.SetNoCreate();

    // First get all creatures.
    std::list<Creature*> creatures;
    WCloud::AllFriendlyCreaturesInGrid creature_check(me);
    WCloud::CreatureListSearcher<WCloud::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <WCloud::CreatureListSearcher<WCloud::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
{
    CellCoord p(WCloud::ComputeCellCoord(x, y));
    Cell cell(p);
    cell.SetNoCreate();

    WCloud::RespawnDo u_do;
    WCloud::WorldObjectWorker<WCloud::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 i)
{
    if (i == 1 || (i == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
            <WCloud::CreatureListSearcher<WCloud::AllFriendlyCreaturesInGrid>,
