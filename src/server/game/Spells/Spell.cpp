...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    CellCoord p(WCloud::ComputeCellCoord(m_caster->GetPositionX(), m_caster->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();
    ...
    WCloud::WorldObjectSearcher<T> searcher(m_caster, result, u_check);

    TypeContainerVisitor<WCloud::WorldObjectSearcher<T>, GridTypeMapContainer > grid_searcher(searcher);
    cell.Visit(p, grid_searcher, *m_caster->GetMap(), *m_caster, max_range);

    if (!result)
    {
        TypeContainerVisitor<WCloud::WorldObjectSearcher<T>, WorldTypeMapContainer > world_searcher(searcher);
...
void Spell::SearchChainTarget(std::list<Unit*> &TagUnitMap, float max_range, uint32 num, SpellTargets TargetType)
{
    ...
    while (num)
    {
        ...
        {
            tempUnitMap.sort(WCloud::ObjectDistanceOrderPred(cur));
            next = tempUnitMap.begin();
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    WCloud::SpellNotifierCreatureAndPlayer notifier(m_caster, TagUnitMap, radius, type, TargetType, pos, entry, m_spellInfo);
...
void Spell::SearchGOAreaTarget(std::list<GameObject*> &TagGOMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    WCloud::GameObjectInRangeCheck check(pos->m_positionX, pos->m_positionY, pos->m_positionZ, radius, entry);
    WCloud::GameObjectListSearcher<WCloud::GameObjectInRangeCheck> searcher(m_caster, TagGOMap, check);
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargets TargetType, SpellEffIndex effIndex)
{
    switch (TargetType)
    {
        ...
        {
            Unit* target = NULL;
            WCloud::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            WCloud::UnitLastSearcher<WCloud::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
        ...
        {
            Unit* target = NULL;
            WCloud::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            WCloud::UnitLastSearcher<WCloud::AnyFriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
            m_caster->VisitNearbyObject(range, searcher);
            return target;
        }
    }
    ...
    else if (pushType)
    {
            ...
            case SPELL_TARGETS_ENTRY:
            {
                ...
                else
                {
                    ...
                    switch (m_spellInfo->Id)
                    {
                        ...
                        {
                            if (WorldObject* result = FindCorpseUsing<WCloud::RaiseDeadObjectCheck> ())
                            {
                        ...
                        case 51328:
                                    ...
                                    && m_targets.GetUnitTarget()->GetDisplayId() == m_targets.GetUnitTarget()->GetNativeDisplayId()))))
                            {
                                ...
                                WorldObject* result = FindCorpseUsing <WCloud::ExplodeCorpseObjectCheck> ();
        ...
        if (!unitList.empty())
        {
            ...
            if (maxSize && power != -1)
            {
                if (Powers(power) == POWER_HEALTH)
                {
                    ...
                    {
                        unitList.sort(WCloud::HealthPctOrderPred());
                ...
                else
                {
                    ...
                    if (unitList.size() > maxSize)
                    {
                        unitList.sort(WCloud::PowerPctOrderPred((Powers)power));
                        unitList.resize(maxSize);
                    }
                }
            }
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                WCloud::RandomResizeList(unitList, maxTargets);
            }
        ...
        if (!gobjectList.empty())
        {
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                WCloud::RandomResizeList(gobjectList, maxTargets);
            }
...
SpellCastResult Spell::CheckItems()
{
    ...
    if (m_spellInfo->RequiresSpellFocus)
    {
        ...
        GameObject* ok = NULL;
        WCloud::GameObjectFocusCheck go_check(m_caster, m_spellInfo->RequiresSpellFocus);
        WCloud::GameObjectSearcher<WCloud::GameObjectFocusCheck> checker(m_caster, ok, go_check);

        TypeContainerVisitor<WCloud::GameObjectSearcher<WCloud::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
...
void Spell::SelectTrajTargets()
{
    ...
    unitList.sort(WCloud::ObjectDistanceOrderPred(m_caster));
