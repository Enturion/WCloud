...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = WCloud::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = WCloud::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = WCloud::ComputeCellCoord(x + radius, y + radius).normalize();
