...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellCoord cellCoord = WCloud::ComputeCellCoord(data->posX, data->posY);
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!WCloud::IsValidMapCoord(tmp.TeleportTo.DestX, tmp.TeleportTo.DestY, tmp.TeleportTo.DestZ, tmp.TeleportTo.Orientation))
                {
            ...
            {
                if (!WCloud::IsValidMapCoord(tmp.TempSummonCreature.PosX, tmp.TempSummonCreature.PosY, tmp.TempSummonCreature.PosZ, tmp.TempSummonCreature.Orientation))
                {
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        if (!WCloud::IsValidMapCoord(POI.x, POI.y))
        {
...
bool ObjectMgr::LoadWCloudStrings(char const* table, int32 min_value, int32 max_value)
{
    uint32 oldMSTime = getMSTime();
    ...
    for (WCloudStringLocaleMap::iterator itr = mWCloudStringLocaleMap.begin(); itr != mWCloudStringLocaleMap.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
            mWCloudStringLocaleMap.erase(itr++);
        else
            ++itr;
    }
    ...
    if (!result)
    {
        if (min_value == MIN_SKYFIRE_STRING_ID)              // error only in case internal strings
            sLog->outErrorDb(">> Loaded 0 WCloud strings. DB table `%s` is empty. Cannot continue.", table);
        else
    ...
    do
    {
        ...
        WCloudStringLocale& data = mWCloudStringLocaleMap[entry];
    ...
    if (min_value == MIN_SKYFIRE_STRING_ID)
        sLog->outString(">> Loaded %u WCloud strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetWCloudString(int32 entry, LocaleConstant locale_idx) const
{
    if (WCloudStringLocale const *msl = GetWCloudStringLocale(entry))
    {
    ...
    if (entry > 0)
        sLog->outErrorDb("Entry %i not found in `wcloud_string` table.", entry);
    else
        sLog->outErrorDb("WCloud string entry %i is not found in DB.", entry);
    return "<error>";
}
    ...
    for (ScriptMapMap::const_iterator itrMM = scripts->begin(); itrMM != scripts->end(); ++itrMM)
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            switch (itrM->second.command)
            {
                case SCRIPT_COMMAND_TALK:
                {
                    if (!GetWCloudStringLocale (itrM->second.Talk.TextID))
...
{
    LoadWCloudStrings("db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (GetWCloudStringLocale(i))
            ids.insert(i);
...
bool LoadWCloudStrings(char const* table, int32 start_value, int32 end_value)
{
    // MAX_DB_SCRIPT_STRING_ID is max allowed negative value for scripts (scrpts can use only more deep negative values
    // start/end reversed for negative values
    if (start_value > MAX_DB_SCRIPT_STRING_ID || end_value >= start_value)
    {
        sLog->outErrorDb("Table '%s' load attempted with range (%d - %d) reserved by WCloud, strings not loaded.", table, start_value, end_value+1);
        return false;
    }

    return sObjectMgr->LoadWCloudStrings(table, start_value, end_value);
}
