...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data)
{
    ...
    else
    {
        ...
        if (!sender->CanSpeak())
        {
            ...
            SendNotification(GetWCloudString(LANG_WAIT_BEFORE_SPEAKING), timeStr.c_str());
    ...
    if (sender->HasAura(1852) && type != CHAT_MSG_WHISPER)
    {
        ...
        SendNotification(GetWCloudString(LANG_GM_SILENCE), sender->GetName());
        return;
    }
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
                SendNotification(GetWCloudString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            {
                SendNotification(GetWCloudString(LANG_WHISPER_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }
            ...
            {
                SendNotification(GetWCloudString(LANG_GM_SILENCE), GetPlayer()->GetName());
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (AccountMgr::IsPlayerAccount(GetSecurity()))
            {
                ...
                {
                    SendNotification(GetWCloudString(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                    return;
                }
            }
        ...
        case CHAT_MSG_AFK:
        {
            if ((msg.empty() || !sender->isAFK()) && !sender->isInCombat())
            {
                ...
                {
                    if (msg.empty())
                        msg  = GetWCloudString(LANG_PLAYER_AFK_DEFAULT);
                    sender->afkMsg = msg;
                }
        ...
        case CHAT_MSG_DND:
        {
            if (msg.empty() || !_player->isDND())
            {
                ...
                {
                    if (msg.empty())
                        msg = GetWCloudString(LANG_PLAYER_DND_DEFAULT);
                    _player->dndMsg = msg;
                }
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket & recv_data)
{
    ...
    CellCoord p = WCloud::ComputeCellCoord(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

    Cell cell(p);
    cell.SetNoCreate();

    WCloud::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum, unit);
    WCloud::LocalizedPacketDo<WCloud::EmoteChatBuilder > emote_do(emote_builder);
    WCloud::PlayerDistWorker<WCloud::LocalizedPacketDo<WCloud::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
    TypeContainerVisitor<WCloud::PlayerDistWorker<WCloud::LocalizedPacketDo<WCloud::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
