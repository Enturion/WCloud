...
{
    char const* format = GetWCloudString(string_id);
    if (format)
    {
        va_list ap;
        char szStr [1024];
...
const char *WorldSession::GetWCloudString(int32 entry) const
{
    return sObjectMgr->GetWCloudString(entry, GetSessionDbLocaleIndex());
}
