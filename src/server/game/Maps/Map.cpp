...
{
    CellCoord p = WCloud::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
    CellCoord cellCoord = WCloud::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = WCloud::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<WCloud::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<WCloud::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    WCloud::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<WCloud::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<WCloud::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                WCloud::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
                TypeContainerVisitor<WCloud::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<WCloud::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
{
    cell.SetNoCreate();
    WCloud::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<WCloud::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    WCloud::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<WCloud::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<WCloud::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridCoord p = WCloud::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridCoord p2 = WCloud::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
