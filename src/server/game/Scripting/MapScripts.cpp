...
inline GameObject* Map::_FindGameObject(WorldObject* searchObject, uint32 guid) const
{
    ...
    CellCoord p(WCloud::ComputeCellCoord(searchObject->GetPositionX(), searchObject->GetPositionY()));
    Cell cell(p);

    WCloud::GameObjectWithDbGUIDCheck goCheck(*searchObject, guid);
    WCloud::GameObjectSearcher<WCloud::GameObjectWithDbGUIDCheck> checker(searchObject, gameobject, goCheck);

    TypeContainerVisitor<WCloud::GameObjectSearcher<WCloud::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > objectChecker(checker);
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
                ...
                if (step.script->Talk.Flags & SF_TALK_USE_PLAYER)
                {
                    if (Player* pSource = _GetScriptPlayerSourceOrTarget(source, target, step.script))
                    {
                        ...
                        std::string text(sObjectMgr->GetWCloudString(step.script->Talk.TextID, loc_idx));
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                if (source) //using grid searcher
                {
                    ...
                    CellCoord p(WCloud::ComputeCellCoord(wSource->GetPositionX(), wSource->GetPositionY()));
                    Cell cell(p);

                    WCloud::CreatureWithDbGUIDCheck target_check(wSource, step.script->CallScript.CreatureEntry);
                    WCloud::CreatureSearcher<WCloud::CreatureWithDbGUIDCheck> checker(wSource, cTarget, target_check);

                    TypeContainerVisitor<WCloud::CreatureSearcher <WCloud::CreatureWithDbGUIDCheck>, GridTypeMapContainer > unit_checker(checker);
