...
bool ChatHandler::HandleCooldownCommand(const char *args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == m_session->GetPlayer() ? GetWCloudString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id, true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == m_session->GetPlayer() ? GetWCloudString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleAddItemCommand(const char *args)
{
    ...
    sLog->outDetail(GetWCloudString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(const char *args)
{
    ...
    sLog->outDetail(GetWCloudString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if (skillInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                {
                    knownStr = GetWCloudString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetWCloudString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); id++)
    {
        ...
        if (spellInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (rank)
                    ss << GetWCloudString(LANG_SPELL_RANK) << rank;
                ...
                if (talent)
                    ss << GetWCloudString(LANG_TALENT);
                if (passive)
                    ss << GetWCloudString(LANG_PASSIVE);
                if (learn)
                    ss << GetWCloudString(LANG_LEARN);
                if (known)
                    ss << GetWCloudString(LANG_KNOWN);
                if (active)
                    ss << GetWCloudString(LANG_ACTIVE);
                        ...
                        if (target)
                        {
                            ...
                            if (status == QUEST_STATUS_COMPLETE)
                            {
                                if (target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetWCloudString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetWCloudString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if (status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetWCloudString(LANG_COMMAND_QUEST_ACTIVE);
                        }
        ...
        if (factionEntry)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (repState)                               // and then target != NULL also
                {
                    ...
                    std::string rankName = GetWCloudString(index);

                    ss << ' ' << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ')';

                    if (repState->Flags & FACTION_FLAG_VISIBLE)
                        ss << GetWCloudString(LANG_FACTION_VISIBLE);
                    if (repState->Flags & FACTION_FLAG_AT_WAR)
                        ss << GetWCloudString(LANG_FACTION_ATWAR);
                    if (repState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << GetWCloudString(LANG_FACTION_PEACE_FORCED);
                    if (repState->Flags & FACTION_FLAG_HIDDEN)
                        ss << GetWCloudString(LANG_FACTION_HIDDEN);
                    if (repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << GetWCloudString(LANG_FACTION_INVISIBLE_FORCED);
                    if (repState->Flags & FACTION_FLAG_INACTIVE)
                        ss << GetWCloudString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << GetWCloudString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetWCloudString(LANG_CONTINENT);

                switch (MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetWCloudString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetWCloudString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetWCloudString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetWCloudString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetWCloudString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetWCloudString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char *args)
{
    ...
    if (graveyard)
    {
        ...
        std::string team_name = GetWCloudString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if (g_team == 0)
            team_name = GetWCloudString(LANG_COMMAND_GRAVEYARD_ANY);
        else if (g_team == HORDE)
            team_name = GetWCloudString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if (g_team == ALLIANCE)
            team_name = GetWCloudString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleListAurasCommand (const char * /*args*/)
{
    ...
    char const* talentStr = GetWCloudString(LANG_TALENT);
    char const* passiveStr = GetWCloudString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoCharacterCommand(const char *args)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetWCloudString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetWCloudString(LANG_BANINFO_YES) : GetWCloudString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    }
    while (result->NextRow());
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetWCloudString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetWCloudString(LANG_BANINFO_YES) : GetWCloudString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    } while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char *args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetCString(), fields[1].GetCString(), permanent ? GetWCloudString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
        permanent ? GetWCloudString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
...
bool ChatHandler::HandleRespawnCommand(const char* /*args*/)
{
    ...
    CellCoord p(WCloud::ComputeCellCoord(player->GetPositionX(), player->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    WCloud::RespawnDo u_do;
    WCloud::WorldObjectWorker<WCloud::RespawnDo> worker(player, u_do);

    TypeContainerVisitor<WCloud::WorldObjectWorker<WCloud::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
