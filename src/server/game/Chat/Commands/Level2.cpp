...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    std::string username = GetWCloudString(LANG_ERROR);
    std::string email = GetWCloudString(LANG_ERROR);
    std::string last_ip = GetWCloudString(LANG_ERROR);
    uint32 security = 0;
    std::string last_login = GetWCloudString(LANG_ERROR);
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetWCloudString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, email.c_str(), security, last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleCharacterReputationCommand(const char* args)
{
    ...
    for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
    {
        ...
        std::string rankName = GetWCloudString(ReputationRankStrIndex[rank]);
...
bool ChatHandler::HandleLookupEventCommand(const char* args)
{
    ...
    for (uint32 id = 0; id < events.size(); ++id)
    {
        ...
        if (Utf8FitTo(descr, wnamepart))
        {
            ...
            char const* active = activeEvents.find(id) != activeEvents.end() ? GetWCloudString(LANG_ACTIVE) : "";
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
        ...
        if (titleInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                char const* knownStr = target && target->HasTitle(titleInfo) ? GetWCloudString(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                    ? GetWCloudString(LANG_ACTIVE)
                    : "";
...
bool ChatHandler::HandleCharacterTitlesCommand(const char* args)
{
    ...
    char const* knownStr = GetWCloudString(LANG_KNOWN);
