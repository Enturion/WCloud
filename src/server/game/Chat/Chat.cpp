...
{
    const char *format = GetWCloudString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetWCloudString(int32 entry) const
{
    return m_session->GetWCloudString(entry);
}
...
{
    SendSysMessage(GetWCloudString(entry));
}
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    WCloud::NearestGameObjectCheck check(*pl);
    WCloud::GameObjectLastSearcher<WCloud::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        WCloud::GameObjectWithDbGUIDCheck go_check(*player, lowguid);
        WCloud::GameObjectSearcher<WCloud::GameObjectWithDbGUIDCheck> checker(player, obj, go_check);

        TypeContainerVisitor<WCloud::GameObjectSearcher<WCloud::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
const char *CliHandler::GetWCloudString(int32 entry) const
{
    return sObjectMgr->GetWCloudStringForDBCLocale(entry);
}
...
{
    return GetWCloudString(LANG_CONSOLE_COMMAND);
}
