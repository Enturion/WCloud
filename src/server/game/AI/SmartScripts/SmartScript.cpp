    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST:
        {
            ...
            if (e.action.flee.withEmote)
                sCreatureTextMgr->SendChatString(me, sObjectMgr->GetWCloudStringForDBCLocale(LANG_FLEE), CHAT_MSG_MONSTER_EMOTE);
...
ObjectList* SmartScript::GetWorldObjectsInDist(float dist)
{
    ...
    if (obj)
    {
        WCloud::AllWorldObjectsInRange u_check(obj, dist);
        WCloud::WorldObjectListSearcher<WCloud::AllWorldObjectsInRange> searcher(obj, *targets, u_check);
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    CellCoord p(WCloud::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Unit* unit = NULL;

    WCloud::MostHPMissingInRange u_check(me, range, MinHPDiff);
    WCloud::UnitLastSearcher<WCloud::MostHPMissingInRange> searcher(me, unit, u_check);

    TypeContainerVisitor<WCloud::UnitLastSearcher<WCloud::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    WCloud::FriendlyCCedInRange u_check(me, range);
    WCloud::CreatureListSearcher<WCloud::FriendlyCCedInRange> searcher(me, _list, u_check);

    TypeContainerVisitor<WCloud::CreatureListSearcher<WCloud::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature*>& list, float range, uint32 spellid)
{
    ...
    WCloud::FriendlyMissingBuffInRange u_check(me, range, spellid);
    WCloud::CreatureListSearcher<WCloud::FriendlyMissingBuffInRange> searcher(me, list, u_check);

    TypeContainerVisitor<WCloud::CreatureListSearcher<WCloud::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
